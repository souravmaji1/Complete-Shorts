...
const fetchAdAccounts = () => {
  setLoading(true);
  FB.api('/me/adaccounts', { fields: 'id,name,account_status,business,pages{instagram_business_account}' }, function (response) {
    setLoading(false);
    if (response && !response.error) {
...
const fetchPages = () => {
  setLoading(true);
  FB.api('/me/accounts', { fields: 'id,name,category,instagram_business_account,access_token' }, function (response) {
    setLoading(false);
    if (response && !response.error) {
...
const handleLinkInstagram = () => {
  ...
  FB.login(
    ...
    { scope: 'ads_management,pages_show_list,instagram_basic,instagram_manage_comments,pages_manage_metadata', return_scopes: true }
  );
};
  ...
  const isCurrentStepValid = () => {
    switch (currentStep) {
      ...
      case 4:
        return budget !== '' && !isNaN(budget) && parseFloat(budget) > 0;
      case 5:
                      ...
                      FB.api(
                        ...
                        function (adResponse) {
                          ...
                          if (adResponse && !adResponse.error) {
                            setSuccessMessage('Instagram campaign created successfully! It is paused and ready for review in Meta Ads Manager.');
                            setCampaignName('');
                            setBudget('');
                            setImageUrl('');
                            setImageFile(null);
      ...
      {adAccounts.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {adAccounts.map((account) => (
            ...
            >
              {selectedAdAccount === account.id && (
                <div className="absolute inset-0 border border-amber-500/50 rounded-xl animate-pulse"></div>
              )}
              ...
              <div className="p-5">
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-md ${
                      ...
                      <Briefcase size={16} className={`${selectedAdAccount === account.id ? 'text-white' : 'text-gray-400'}`} />
                    </div>
                    <div className="ml-3">
                ...
                {selectedAdAccount === account.id && (
                  <div className="mt-4 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20 flex items-center animate-pulse">
                    <CheckCircle size={16} className="text-amber-400 mr-2" />
                    <span className="text-sm text-amber-300">Selected for campaign</span>
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="fade-in space-y-4">
      ...
      {pages.length > 0 ? (
            ...
            <div 
              ...
              className={`relative group transition-all duration-300 border rounded-xl overflow-hidden cursor-pointer backdrop-blur-sm ${
                selectedPage === page.id 
                  ? 'border-amber-500 bg-gradient-to-br from-amber-900/20 via-orange-900/20 to-gray-900/80 transform scale-[1.02] shadow-lg shadow-amber-500/20' 
                  : 'border-gray-700/30 bg-gray-900/30 hover:bg-gray-800/50 hover:scale-[1.01] hover:border-amber-500/30'
              }`}
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-sm text-gray-400">Instagram ID: {page.instagram_business_account?.id || 'N/A'}</p>
        ...
        <div className="border border-gray-700/30 rounded-xl bg-gradient-to-br from-gray-900/60 via-slate-900/40 to-black/60 backdrop-blur-sm p-8 text-center shadow-lg">
          ...
          <button 
            ...
            className="mt-4 bg-gradient-to-r from-amber-600 to-orange-600 px-4 py-2 rounded-lg text-sm text-white hover:opacity-90 transition shadow-lg shadow-amber-500/20 transform hover:scale-105"
          >
      ...
      {showLinkModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gradient-to-br from-gray-900/90 to-black/90 rounded-xl p-6 w-full max-w-md border border-gray-700/30 shadow-xl shadow-amber-500/10">
            <h3 className="text-lg font-semibold text-white mb-4">Link Instagram Account</h3>
            <p className="text-sm text-gray-400 mb-4">Select a Facebook Page to link your Instagram Business Account to.</p>
            ...
            <div className="mb-4">
              ...
              <select
                ...
                className="w-full border border-gray-700/30 rounded-xl px-4 py-3 bg-gray-900/60 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 shadow-sm hover:shadow-amber-500/20"
              >
            ...
            {allPages.length === 0 && (
              <p className="text-sm text-red-400 mb-4">No pages available. Create a page in Meta Business Manager first.</p>
            )}
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-amber-600 to-orange-600 rounded-lg text-white hover:opacity-90 transition disabled:opacity-50 shadow-lg shadow-amber-500/20 transform hover:scale-105"
              >
  ...
  const renderCampaignSetup = () => (
    <div className="fade-in space-y-6">
      <h3 className="text-xl font-semibold mb-5 bg-gradient-to-r from-amber-400 via-orange-400 to-yellow-400 bg-clip-text text-transparent flex items-center">
        <div className="relative mr-2">
          <GanttChart size={20} className="text-amber-400" />
          <div className="absolute inset-0 bg-amber-400/20 rounded-full blur-md"></div>
        </div>
        Campaign Setup
      </h3>
      ...
      <div className="space-y-6">
        <div className="relative group">
          ...
          <div className="relative">
            ...
            <div className={`absolute right-3 top-3.5 transition-opacity ${campaignName ? 'opacity-100' : 'opacity-0'}`}>
    ...
    <div className="fade-in space-y-6">
      ...
      <div className="space-y-6">
        <div className="relative">
          ...
          <div className="relative">
            <input
              ...
              min="1"
            />
            <div className="absolute left-3 top-3.5 text-amber-400 font-medium">
              $
            </div>
            ...
            <div className="flex space-x-4">
              <div className="relative w-1/2">
                <select
                  ...
                  className="w-full border border-gray-700/30 rounded-xl px-4 py-3.5 bg-gray-900/60 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all shadow-sm hover:shadow-amber-500/20"
                >
                  {[...Array(48)].map((_, i) => (
        ...
        <div className="bg-gradient-to-br from-amber-900/20 to-gray-900/20 backdrop-blur-sm rounded-xl p-4 border border-amber-500/20 shadow-lg">
          ...
          <p className="text-sm text-gray-400">
            You're targeting <span className="text-amber-300">{audience.countries[0] === 'US' ? 'United States' : audience.countries[0]}</span> users aged <span className="text-amber-300">{audience.ageMin}-{audience.ageMax}</span>. Estimated potential reach: <span className="text-amber-300">100M+ people</span>
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="relative">
          ...
          <div className="relative">
            <textarea
              ...
              className="w-full border border-gray-700/30 rounded-xl px-4 py-3.5 bg-gray-900/60 text-white focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-all resize-none shadow-sm hover:shadow-amber-500/20"
        ...
        <div className="relative">
          ...
          <div className="border-2 border-dashed border-gray-700/30 rounded-xl p-8 text-center bg-gray-900/30 hover:bg-gray-900/50 transition-all cursor-pointer group backdrop-blur-sm">
            ...
            <label htmlFor="image-upload" className="cursor-pointer">
              <div className="w-16 h-16 rounded-full bg-gray-800/60 flex items-center justify-center mx-auto mb-4 group-hover:bg-amber-900/20 transition-all">
                <ImageIcon className="h-8 w-8 text-gray-400 group-hover:text-amber-400 transition-all" />
              </div>
              ...
              <button
                className="mt-4 px-6 py-2 bg-gradient-to-r from-amber-600 to-orange-600 text-white text-sm font-medium rounded-lg shadow-md shadow-amber-500/10 hover:shadow-lg hover:shadow-amber-500/20 transition-all transform hover:scale-105"
                disabled={loading}
              >
              ...
              <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-y border-slate-700/30 shadow-lg">
                <div className="flex items-center px-4 py-3">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-slate-400 text-xs">Sponsored · <span className="text-pink-400 font-medium">Follow</span></p>
                  </div>
                </div>
                ...
                <div className="border-t border-slate-700/50 px-4 py-3 flex justify-between bg-gradient-to-r from-slate-900/50 to-slate-800/50">
                  <div className="flex items-center space-x-6">
                    <div className="flex items-center group cursor-pointer">
                      ...
                      <span className="text-slate-400 text-xs ml-2 group-hover:text-pink-400 transition-colors">Like</span>
                    </div>
                    <div className="flex items-center group cursor-pointer">
                      ...
                      <span className="text-slate-400 text-xs ml-2 group-hover:text-purple-400 transition-colors">Comment</span>
                    </div>
                  </div>
                  <div className="flex items-center group cursor-pointer">
                    ...
                    <span className="text-slate-400 text-xs ml-2 group-hover:text-cyan-400 transition-colors">Share</span>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-slate-950 to-black text-white relative overflow-hidden">
        ...
        <nav className="flex justify-between items-center px-6 py-6 lg:px-20 border-b border-gray-800/50 sticky top-0 z-50 backdrop-blur-2xl bg-gray-950/80 shadow-2xl shadow-black/20">
          <div className="flex items-center group">
            <div className="relative">
              <Sparkles className="text-amber-400 mr-3 group-hover:text-orange-400 transition-colors duration-300" size={28} />
              <div className="absolute inset-0 bg-amber-400/20 rounded-full blur-lg group-hover:bg-orange-400/20 transition-colors duration-300"></div>
            </div>
            <span className="text-2xl font-bold bg-gradient-to-r from-amber-400 via-orange-400 to-yellow-400 bg-clip-text text-transparent">
              InstaGenius AI
            </span>
          </div>
          <div className="hidden md:flex space-x-8">
            {["Features", "Showcase", "Pricing", "Testimonials"].map((item) => (
              <a
                ...
                className="relative hover:text-amber-400 transition-all duration-300 group"
              >
                {item}
          ...
          {isLoggedIn ? (
            <div className="flex items-center gap-4">
              <span className="hidden md:block text-gray-300 px-3 py-1 bg-gray-800/50 rounded-full border border-gray-700/50 backdrop-blur-sm">
                {userName}
              </span>
              <button
                onClick={handleLogout}
                className="bg-gradient-to-r from-gray-800 to-gray-900 border border-gray-600/50 px-6 py-2 rounded-xl hover:from-gray-700 hover:to-gray-800 transition-all duration-300 backdrop-blur-sm shadow-lg hover:shadow-xl transform hover:scale-105"
              >
                Logout
              </button>
            </div>
            ...
            <button
              onClick={handleLogin}
              className="relative bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 px-6 py-3 rounded-xl font-medium hover:shadow-2xl hover:shadow-amber-500/30 transition-all duration-300 overflow-hidden group transform hover:scale-105"
            >
          ...
          {isLoggedIn ? (
            <div className="max-w-7xl mx-auto">
              ...
              <div className="text-center mb-16">
                ...
                <p className="mt-6 text-xl text-gray-300 font-light">Follow the steps to launch your campaign in minutes</p>
              </div>
                              ...
                              >
                                <span className="text-gray-300 text-sm flex items-center">
                                  <div className="relative mr-3">
                                    <Icon
                                      size={18}
                                      className="text-amber-400 group-hover:text-orange-400 transition-colors duration-300"
                                    />
            ...
            <div className="relative z-10 max-w-5xl mx-auto text-center">
              <div className="relative">
                <div className="absolute -inset-8 bg-gradient-to-r from-amber-600/20 via-orange-600/20 to-yellow-600/20 rounded-[4rem] blur-3xl"></div>
                <h1 className="relative text-5xl sm:text-6xl lg:text-7xl font-bold mb-8 leading-tight">
                  ...
                  <span className="bg-gradient-to-r from-yellow-400 via-amber-400 to-orange-400 bg-clip-text text-transparent block mt-2">
                    In Minutes, Not Hours
                  </span>
                </h1>
              </div>
              <p className="text-xl lg:text-2xl text-gray-300 font-light max-w-3xl mx-auto leading-relaxed mb-12">
                Our AI-powered platform connects directly to your Instagram account,
                streamlines campaign creation, and optimizes for maximum ROI with intelligent targeting and creative suggestions.
              </p>
              <div className="flex flex-col sm:flex-row gap-6 justify-center items-center mb-16">
                <button
                  onClick={handleLogin}
                  className="group relative bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 px-12 py-6 rounded-2xl font-bold text-lg hover:shadow-2xl hover:shadow-amber-500/40 transition-all duration-300 overflow-hidden transform hover:scale-105"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-700 via-orange-700 to-yellow-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                  <span className="relative z-10 flex items-center">
                ...
                <div className="flex items-center text-gray-400">
                  ...
                  <span className="text-sm">
                    Join <span className="text-amber-400 font-semibold">10,000+</span> marketers
                  </span>
                </div>
              </div>
              {/* Feature Grid */}
                ...
                {[
                  {
                    ...
                    description: "Create professional campaigns in under 5 minutes with our guided wizard",
                  },
                  {
                    icon: Target,
                    title: "Smart Targeting",
                    description: "AI-powered audience optimization based on your business goals and data",
                  },
                  {
                    icon: TrendingUp,
                    title: "Real-time Analytics",
                    description: "Monitor performance and get actionable insights to maximize your ROI",
                  },
                ].map((feature, index) => (
                  <div key={index} className="group relative">
                    <div className="absolute -inset-2 bg-gradient-to-br from-amber-600/10 via-orange-600/10 to-yellow-600/10 rounded-2xl blur-xl group-hover:blur-2xl transition-all duration-500 opacity-0 group-hover:opacity-100"></div>
                    <div className="relative bg-gradient-to-br from-gray-900/60 via-slate-900/40 to-black/60 backdrop-blur-2xl border border-gray-700/30 rounded-2xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 transform group-hover:scale-105">
                      <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 via-transparent to-orange-500/5 rounded-2xl"></div>
                      <div className="relative z-10">
                        <div className="relative mb-6">
                          <feature.icon
                            ...
                            className="text-amber-400 group-hover:text-orange-400 transition-colors duration-300"
                          />
                          <div className="absolute inset-0 bg-amber-400/20 rounded-full blur-lg group-hover:bg-orange-400/20 transition-all duration-300"></div>
                        </div>
              ...
              <div className="fixed bottom-8 right-8 z-50">
                <div className="group relative">
                  <div className="absolute -inset-4 bg-gradient-to-r from-amber-600/30 via-orange-600/30 to-yellow-600/30 rounded-full blur-xl group-hover:blur-2xl transition-all duration-500"></div>
                  <button className="relative bg-gradient-to-r from-amber-600 via-orange-600 to-yellow-600 p-4 rounded-full shadow-2xl hover:shadow-amber-500/40 transition-all duration-300 transform hover:scale-110">
