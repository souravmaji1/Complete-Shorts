...
async function createCampaign(options) {
    const { customerId, refreshToken, campaignDetails, managerCustomerId } = options;
  ...
  try {
    ...
    const campaignOperations = [
      {
        ...
        resource: {
          ...
          start_date: campaignDetails.startDate, // Add start date
          end_date: campaignDetails.endDate // Add end date
        }
      }
    ];
    ...
    const imageUrl = 'https://skezvvinpjzxdpidbglf.supabase.co/storage/v1/object/public/fgfg/uploads/user_2xR60F9U4ngOHCrWs0NbsrjviHI/logos/logo_1754554009087_TS2W74VR_400x400.jpg';
    ...
    // Step 5: Create ad
    console.log('Creating DemandGenVideoResponsiveAd...');
    const adOperations = [
      {
        entity: 'ad_group_ad',
        operation: 'create',
        resource: {
          ...
          ad: {
            ...
            demand_gen_video_responsive_ad: {
              headlines: [
                { text: 'Watch Our Amazing Video!' },
                { text: 'Discover Now on YouTube' },
                { text: 'Engaging Content Awaits' }
              ],
              long_headlines: [
                { text: 'This 27 Kitchen Gadget Is Replacing Expensive Appliances in Millions of Homes Worldwide' }
              ],
              descriptions: [
                { text: 'Meet the revolutionary 6-in-1 kitchen tool transforming family cooking and prep.' },
                { text: 'Is the job market\'s degree requirement a myth? Discover the truth.' }
              ],
              videos: [
...
export async function POST(request) {
  try {
    ...
    const result = await createCampaign({
      ...
      refreshToken,
      campaignDetails,
      managerCustomerId // Make sure this is passed through
    });
