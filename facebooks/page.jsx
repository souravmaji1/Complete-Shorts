...
const MessageBubble = ({ message, isUser }) => {
  const renderToolPart = (part, i, messageId) => {
    if (part.type === 'text') return <span key={i}>{part.text}</span>;

    if (part.type === 'tool-call') {
      switch (part.status) {
        case 'input-available':
          return <div key={i}>🔍 Searching domains…</div>;
        case 'output-available':
          return (
            <ToolCallBox key={i} toolCall={part} />
          );
        case 'output-error':
      ...
      <div className={`max-w-2xl ${isUser ? "items-end" : "items-start"} flex flex-col`}>
        ...
        >
          ...
          {message.parts && message.parts.map((part, i) => 
            renderToolPart(part, i, message.id)
          )}
        </div>
    ...
    initialMessages: [
      {
        id: uuidv4(),
        role: "assistant",
        content: `🎯 Welcome to DomainHunter Pro! I'm your AI-powered assistant for finding and analyzing expired domains.\n\n✨ Here's what I can help you with:\n\n🔍 Finding expired domains with high SEO value\n📊 Analyzing domain metrics (DA, PA, backlinks)\n💡 Suggesting valuable domain opportunities\n🚀 Checking domain availability and pricing\n\nI'll help you discover hidden gems in the expired domain market. What would you like to search for today?`,
      },
    ],
    onError: (error) => {
          ...
          {error && (
            <div className="mx-6 mb-4">
              <div className="max-w-4xl mx-auto bg-red-950/30 border border-red-500/20 rounded-xl p-4 backdrop-blur-sm">
                ...
                <div className="text-sm text-red-300 whitespace-pre-wrap">{error.message}</div>
              </div>
            </div>
          )}
